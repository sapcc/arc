...
C:\\monsoon\\arc\\arc.exe init --endpoint {{ .EndpointURL }} --update-uri {{ .UpdateURL }} --registration-url {{ .SignURL }} {{if .RenewCertURL}} --renew-cert-uri {{ .RenewCertURL }} {{end}}
`))
...
chmod +x /opt/arc/arc
/opt/arc/arc init --endpoint {{ .EndpointURL }} --update-uri {{ .UpdateURL }} --registration-url {{ .SignURL }} {{if .RenewCertURL}}--renew-cert-uri {{ .RenewCertURL }} {{end}}
`))
